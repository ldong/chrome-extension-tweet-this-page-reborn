  ...
  chrome.extension.sendRequest({method: 'get_access_token', 'pin': pin},
    function(response) {
      ...
      if (response.status == 'SUCCESS') {
        setMessage('Authorized! You can close this page, the extension is ready to use. Or it will be closed in 3 seconds');
        setTimeout(function(){
          chrome.extension.sendRequest({method:"closePage"});
        }, 3000);
      } else {
