...
// Maximum length of shortened URL. if use bitlyLogin, it's 23, if not, it's 22
TwitterUtils.SHORTENED_URL_MAX_LENGTH = localStorage.bitlyLogin ? 23 : 22;
  ...
  if (getBooleanOption(OPTION_TWEET_SELECTION)) {
    chrome.tabs.getSelected(null, function(tab) {
      if (!tab.url.match('^' + TwitterUtils.URL_SCRIPTS_DISALLOWED)) {
        chrome.tabs.executeScript(tab.id, {file: 'js/selection.js'}, function() {
